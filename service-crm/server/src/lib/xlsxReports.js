...
function addTitleBlock(sheet, title, filterLines) {
...
function addDataTable(sheet, columns, rows, startRowOffset = 0) {
...
function addSectionHeading(sheet, text) {
  const row = sheet.addRow([text]);
  row.font = SECTION_FONT;
  sheet.addRow([]);
}

function filterSummaryLines({ from, to, extra = [] }) {
